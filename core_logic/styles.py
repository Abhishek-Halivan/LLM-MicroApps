...
def get_colorful_styles():
    """
    Alternative vibrant and colorful theme for the app.
    Use by calling `st.markdown(get_colorful_styles(), unsafe_allow_html=True)` instead of the default.
    """
    return """
    <style>
        /* Playful & colorful theme */
        :root{
            --bg-1: #ffecd2; /* soft peach */
            --bg-2: #fcb69f; /* warm coral */
            --accent-a: #7f5af0; /* purple */
            --accent-b: #ffb86b; /* gold */
            --accent-c: #00d4ff; /* cyan */
            --text-1: #0b1220; /* deep text */
            --muted-1: #4b5563;
            --card: #ffffff;
            --glass: rgba(255,255,255,0.65);
            --radius: 14px;
            --pop: 0 10px 30px rgba(127,90,240,0.12);
        }

        html,body{
            min-height:100%;
            background: radial-gradient(1200px 600px at 10% 10%, var(--bg-1) 0%, transparent 25%),
                        linear-gradient(120deg, var(--bg-1) 0%, var(--bg-2) 45%, #ffd1e6 100%);
        ...
        h1{ font-size:40px; color:var(--accent-a); font-weight:800; margin:0 0 8px 0; }
        h2{ font-size:30px; color:var(--accent-b); margin:18px 0; font-weight:700; }
        h3{ font-size:22px; color:var(--accent-c); }

        /* Cards with colorful border accents */
        .stCard, [data-testid="stExpander"], .card{
        ...
        button.secondary{ background: linear-gradient(90deg,#ffb86b,#ffd86b); color:#1a1a1a; }
        button:hover{ transform:translateY(-4px); box-shadow:0 18px 40px rgba(0,0,0,0.12); }

        label{ font-size:16px; color:var(--text-1); font-weight:700; margin-bottom:8px; display:block; }
        ...
        input:focus, textarea:focus{ box-shadow:0 8px 20px rgba(127,90,240,0.08); transform:translateY(-2px); outline:none; }

        /* Code blocks vibrant */
        code{ background:#fff7f3; color:#7f5af0; padding:0.2rem 0.4rem; border-radius:6px; }
        pre{ background:#0b1220; color:#e6f7ff; padding:14px; border-radius:12px; overflow:auto; }

        /* Alerts */
        [data-testid="stAlert"]{ background:linear-gradient(90deg, rgba(255,255,255,0.9), rgba(255,255,255,0.85)); border-left:5px solid var(--accent-b); padding:12px; border-radius:10px; }

        /* Small text */
        small, .stCaption{ color:var(--text-1); opacity:0.8; }
